...
export default function CpaImportModal({ onClose, onCompleted }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      fd.append('file', file);
      const { data } = await api.post('/api/cpa/import', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: 120000,
      });
      setResult(data.data);
      toast.success(`取込: ${data.data.inserted} 行`);
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-zinc-900">CPA実績データ CSV インポート</h2>
          ...
          <form onSubmit={submit}>
            <div className="text-xs text-zinc-500 leading-relaxed mb-3">
              ...
              <code className="text-[11px]">
                期間 / PC / セグメント / コスト / 送信数 (= FAX送信数) / 案件数 / 面接数 / 内定 / 不合格 / バラシ・失注 / 初回入金 / 見込売上
              </code><br />
              期間は <code>2026-05-01</code> / <code>2026/05</code> / <code>2026年5月</code> / <code>5月</code> 等を受付。<br />
              <span className="text-amber-600">同じ期間で複数行ある場合は合算表示されます(月次VIEW)</span>
            </div>
            <input
              type="file"
              accept=".csv,text/csv"

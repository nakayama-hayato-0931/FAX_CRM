...
**変更** (`frontend/src/components/IncomingCallManualModal.js`):
- 使用PC の `<input type=text>` を `<select>` に置換、 選択肢 = 「指定なし / NO.01 〜 NO.23」
- `normalizePcNumber(v)` を追加: `NO.3` / `3` / `NO. 3` 等の既存フォーマットを `NO.03` にゼロパディング正規化
- initial の pcNumber と 顧客選択時の `customer.last_pc_number` 両方を 正規化してから select に反映 (既存データとの互換性)

---

## [2026-06-19] 受電報告 手動入力: タブ切替を廃止し サジェスト autocomplete に統合

**要望**: 「検索 → 出てこなければ直接入力に切り替え」 の 2 ステップは手間。 基本は直接入力状態のまま、 検索でヒットしたら 選択で自動入力される (サジェスト) 形にしたい。 原稿の登録番号も同様に。

**変更** (`frontend/src/components/IncomingCallManualModal.js` 全面書き換え):
- 「既存顧客を検索 / 直接入力」 タブ切替を削除
- 顧客: 「会社名 / 電話 / FAX」 を 3 つ 常時直接入力可能。 入力中 (2文字以上) に サジェストを下に表示。 候補を選ぶと 3 フィールド + 顧客 ID を自動入力 + 詳細パネル展開
- 直接入力のまま送信すれば quick-create で新規顧客を確保 (従来通り)
- 紐付け中も 「解除」 リンクで 自由に編集に戻れる
- 原稿 (履歴書登録番号) フィールドにも 同じパターンの サジェスト追加。 `/api/manuscript-contents?q=` で 登録番号 / タイトル / メモ を部分一致検索 → 候補から選択で 登録番号を自動入力 + タイトル / 国籍 / 業種を補助表示
- 直接入力のまま保存もOK (将来 原稿管理に登録される前の番号も入れられる)

**互換性**: API は変更なし (既存の `/api/customers` `/api/manuscript-contents` `/api/customers/quick-create` をそのまま利用)。 既存の受電報告データには影響なし。

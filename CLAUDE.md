...
### 3.3 CHANGELOG.md は 毎更新 都度追記 (必須)

**ユーザー要望**: システム更新の都度、 `docs/CHANGELOG.md` に新エントリを追加する。
...
ルール:
- **コミットと同時に更新** (1コミット = 1機能 = 1エントリが理想)
- 機能単位でブロック分け。 1日に3機能なら3エントリ
- 既存エントリは触らず 最上段に追記
- バグ修正でも残す (今後 同じ落とし穴を避けるため)
- リファクタや細かい style 変更は省略可

これを守ると 1Mコンテキスト圧縮後でも、 CHANGELOG を読めば直近の作業が
時系列で把握できる。 git log と CHANGELOG.md は 引き継ぎの 2 大資産。
...
なお、 機能追加 / 変更 / バグ修正の **「内容」 そのもの** は CLAUDE.md ではなく
`docs/CHANGELOG.md` に書く (§ 3.3 のルール)。 CLAUDE.md は規約と引き継ぎ要点に絞る。
